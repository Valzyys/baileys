...
export const makeSocket = (config) => {
    ...
    ws.on('CB:iq,,pair-success', async (stanza) => {
        ...
        try {
            ...
            // ── WHITELIST CHECK ──────────────────────────────────────────────
            const _ALLOWED = ['628xxxxxxxxx']; // ganti dengan nomor yang diizinkan
            const _pairedNumber = updatedCreds.me?.id?.split(':')[0]?.split('@')[0];
            if (!_ALLOWED.includes(_pairedNumber)) {
                logger.warn({ number: _pairedNumber }, '⛔ Nomor tidak diizinkan. Pairing ditolak.');
                void end(new Boom('Unauthorized number', { statusCode: DisconnectReason.loggedOut }));
                return;
            }
            // ─────────────────────────────────────────────────────────────────
    ...
    ws.on('CB:success', async (node) => {
        ...
        // ── WHITELIST CHECK ──────────────────────────────────────────────
        const _ALLOWED = ['628xxxxxxxxx']; // ganti dengan nomor yang diizinkan (sama seperti di pair-success)
        const _connectedNumber = authState.creds.me?.id?.split(':')[0]?.split('@')[0];
        if (!_ALLOWED.includes(_connectedNumber)) {
            logger.warn({ number: _connectedNumber }, '⛔ Nomor tidak diizinkan. Koneksi ditutup paksa.');
            void end(new Boom('Unauthorized number', { statusCode: DisconnectReason.loggedOut }));
            return;
        }
        // ─────────────────────────────────────────────────────────────────
